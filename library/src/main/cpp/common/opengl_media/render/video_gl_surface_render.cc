...
bool VideoGLSurfaceRender::Init(int width, int height) {
	...
	vertex_coords_ = glGetAttribLocation(prog_id_, "position");
...
//		LOGI("xOffset is %.3f", xOffset);
	...
	glVertexAttribPointer(vertex_coords_, 2, GL_FLOAT, 0, 0, _vertices);
	glEnableVertexAttribArray(vertex_coords_);
	static const GLfloat texCoords[] = {xOffset, (GLfloat) (1.0 - yOffset), (GLfloat) (1.0 - xOffset),
										(GLfloat) (1.0 - yOffset), xOffset, yOffset,
...
//		LOGI("xOffset is %.3f", xOffset);
	...
	glVertexAttribPointer(vertex_coords_, 2, GL_FLOAT, 0, 0, _vertices);
	glEnableVertexAttribArray(vertex_coords_);
	static const GLfloat texCoords[] = {xOffset, yOffset, (GLfloat) (1.0 - xOffset), yOffset, xOffset,
										(GLfloat) 1.0 - yOffset,
...
void VideoGLSurfaceRender::RenderToCroppedTexture(GLuint inputTexId, GLuint outputTexId, int originalWidth,
	...
	LOGI("originalWidth is %d originalHeight is %d backing_width_ is %d backing_height_ is %d", originalWidth,
		 originalHeight, backing_width_, backing_height_);
	glViewport(backing_left_, backing_top_, backing_width_, backing_height_);
