...
internal class Camera2(context: Context, private val mCameraCallback: CameraCallback) :
    TCamera {
...
//      val bytes = ByteArray(buffer.capacity())
  ...
  private fun createImageReader(width: Int, height: Int) {
    ...
    mImageReader?.setOnImageAvailableListener({
      try {
        ...
        mCameraCallback.onPreviewCallback(yuv, image.width, image.height, PreviewFormat.NV21)
        image.close()
  ...
  private fun startPreview(surfaceTexture: SurfaceTexture, width: Int, height: Int) {
    if (!isCameraOpened()) {
      return
    }
    ...
    try {
      ...
      mCameraManager.openCamera(mCameraId, object: CameraDevice.StateCallback() {
        override fun onOpened(camera: CameraDevice?) {
          ...
          mCamera = camera
          startPreview(surfaceTexture, size.width, size.height)
          mCameraCallback.onCameraFeatureSupported(isAutoExposureSupported(),
              true,
              true,
              true,
              true,
              ...
              isFlashSupported())
          mCameraCallback.onCameraOpened(size.width, size.height)
        }
