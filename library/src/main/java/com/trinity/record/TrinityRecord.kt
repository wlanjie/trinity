...
class TrinityRecord(
  ...
  override fun createSurface(surface: Surface, width: Int, height: Int) {
    startPreview(surface, width, height, mCameraFacingId)
  }
  ...
  private fun startPreview(surface: Surface, width: Int, height: Int, cameraFacingId: Int) {
    if (mFirst) {
      ...
      com.tencent.mars.xlog.Log.d("tag", "startPreview")
      mFirst = false
    } else {
      // 直接创建EGLSurface
...
//    mAudioPlayer?.seek()
   ...
   */
  private external fun prepareEGLContext(handle: Long, surface: Surface, width: Int, height: Int, cameraFacingId: Int)
